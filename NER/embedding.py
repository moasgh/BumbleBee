...
class embed(nn.Module):
    def __init__(self, cti_size, wti_size, params, hre = False ):
        ...
        # architecture
        for model, dim in self.EMBED.items():
            if model == "char-cnn":
        ...
        if self.hre:
            self.sent_embed = self.rnn(self.EMBED_SIZE, self.EMBED_SIZE, True)
        self = self.cuda(ACTIVE_DEVICE) if CUDA else self

    def forward(self, xc, xw):
        hc, hw = None, None
        if "char-cnn" in self.EMBED or "char-rnn" in self.EMBED:
            hc = self.char_embed(xc)
        if "lookup" in self.EMBED or "sae" in self.EMBED:
